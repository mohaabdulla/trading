...
def run_simulation(data_dict, spy_df, atr_stop, atr_target, trend_filter, rsi_lower, risk_pct=0.02):
    capital = 1000.0
            ...
            for symbol, df in data_dict.items():
                if date in df.index:
                    ...
                    try:
                        ...
                        # Exhaustion Vol
                        if current_vol > avg_vol: continue
...
if __name__ == "__main__":
    ...
    trend_filters = ["medium", "strict"]
    rsi_lowers = [35, 40, 45]
    
    results = []
    print("Running PF=2.0 Optimizer...")
    for s in atr_stops:
        for t in atr_targets:
            for tf in trend_filters:
                for r in rsi_lowers:
                    res = run_simulation(data_dict, spy_df, s, t, tf, r)
                    if res['trades'] >= 10:
                        results.append({
                            'stop': s, 'target': t, 'trend': tf, 'rsi': r,
                            'pf': res['pf'], 'trades': res['trades'],
                            'win_rate': res['win_rate'], 'net': res['net_profit']
                        })
                        
    results.sort(key=lambda x: x['pf'], reverse=True)
    for r in results[:10]:
        print(f"Stop: {r['stop']} ATR | Target: {r['target']} ATR | Trend: {r['trend']:7} | RSI: {r['rsi']} -> PF: {r['pf']:.2f} | Trades: {r['trades']} | WR: {r['win_rate']:.1%} | Net: ${r['net']:.2f}")
